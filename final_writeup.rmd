...
```
Notably, the simplest model has won out. The next step in our process was to identify possible interaction terms. We started with a very complex model with a complete set of interactions terms, which included triple interaction terms across the three predictors, and included log, square root, and inverse terms. We then performed backwards model selection using LOOCV MSE and MAE to select predictors and both metric agreed on the following models for both the first and second moments.

```{r}
model_m1 = glm(R_moment_1 ~ log(St)*Fr*Re + St*Fr*Re, data = clean)
model_m2 = glm(R_moment_1 ~ log(St)*Fr*Re + St*Fr*Re, data = clean)
```
