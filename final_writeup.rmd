...
The first noticeable thing about the data is that despite both Fr and Re being numerical predictors, they only have three levels each. Therefore, we decided to treat them as discrete predictors instead of interpolating or extrapolating values outside the given set for Re ({90, 224, 398}) and Fr ({0.052, 0.3, Inf}). This transformation was appropriate in the context of the problem since the testing data would also have values within the specified sets only.
